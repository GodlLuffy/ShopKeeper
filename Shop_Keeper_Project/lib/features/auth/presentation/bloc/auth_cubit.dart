...
import 'package:shop_keeper_project/services/auth_service.dart';
...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<bool> _checkEmailVerification() async {
    // Development bypass: Since the email is currently unverified
    // (and we can't verify it), we will force this to return true
    // so you can instantly access the dashboard.
    return true;
  }
  ...
  Future<void> sendPasswordResetEmail(String email) async {
    ...
    final actionCodeSettings = authService.brandedActionCodeSettings;
    ...
    result.fold(
      (failure) => emit(AuthError(failure.message)),
      (_) => emit(PasswordResetSent(email: email)),
    );
  }

  Future<void> retryPasswordReset() async {
    if (_lastResetEmail != null) {
      await sendPasswordResetEmail(_lastResetEmail!);
    }
  }
